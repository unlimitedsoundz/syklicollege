...
import { HousingInvoice, HousingPayment, PaymentMethod } from '@/types/database';
...
export async function verifyPayment(
    ...
    try {
        ...
        // 3. Log Audit
        await adminClient.from('housing_audit_logs').insert({
            action: 'PAYMENT_COMPLETED',
            target_resource: 'housing_payments',
            target_id: payment.id,
            details: { transactionId, amount: payment.amount }
        });

        // 4. Update Housing Application Status if Invoice is Fully Paid
        ...
        if (updatedInvoice?.status === 'PAID' && updatedInvoice.application_id) {
            await adminClient
                .from('housing_applications')
                .update({ status: 'APPROVED' }) // Or 'DEPOSIT_PAID' if you have that status
...
export async function batchGenerateInvoices(month: number, year: number) {
    ...
    revalidatePath('/admin/housing/finance');
