...
export async function generateMetadata({ params }: Props) {
    ...
    const schoolName = (dept as any)?.school ? (Array.isArray((dept as any).school) ? (dept as any).school[0]?.name : (dept as any).school.name) : 'School of Arts';
...
export default async function ArtsDepartmentDetailPage({ params }: Props) {
    const resolvedParams = await params;
    const { dept_slug } = resolvedParams;
    const supabase = await createClient();
    const schoolSlug = 'arts'; // Hardcoded for this folder
    ...
    // Verify school slug match
    // The database slug might be 'school-of-arts-design-architecture' or 'arts'.
    // We check if it includes 'arts'.
    if (!dept.school.slug.includes('arts')) {
        // Optionally handle incorrect school association
        // notFound();
    }
    ...
    if (!deptColors[dept.slug]) {
        if (dept.slug.includes('arts')) heroColor = '#e31837';
    }
